...
/**
 * A chat tool is a pi-agent tool plus the set of client query caches its
 * execution dirties. Co-locating `invalidates` with the tool keeps a tool's
 * behaviour and its side effects in one place: adding or renaming a tool can no
 * longer silently desync from a separate name-keyed lookup table.
 */
/**
