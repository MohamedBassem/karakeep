    ...
    const worker = (await getQueueClient())!.createRunner<ZFeedRequestSchema>(
      FeedQueue,
      {
        ...
        onComplete: async (job) => {
          ...
          const feed = await feedsService.getForSystem(job.data?.feedId);
          if (feed) await feedsService.updateLastFetched(feed, "success");
        },
        onError: async (job) => {
          ...
          if (job.data) {
            const feed = await feedsService.getForSystem(job.data.feedId);
            if (feed) await feedsService.updateLastFetched(feed, "failure");
          }
        },
      },
      {
...
async function run(req: DequeuedJob<ZFeedRequestSchema>) {
  const jobId = req.id;
  const feedsService = new FeedsService(db);
  const feed = await feedsService.getForSystem(req.data.feedId);
  if (!feed) {
    throw new Error(
  ...
  await feedsService.updateLastSuccessfulFetchAt(feed);
